...
static void ws_output_write_function(const char * data,size_t len,void *arg)
{
...
int http_ws_handle_connect(http_connection *c)
{
	WS_DBG("http_ws_handle_connect c =%p",c);
	...
	if (strcasecmp(upgrade_header->value,"websocket")!=0) goto badrequest;
	...
	SHA1_CTX ctx;
	SHA1Init(&ctx);
	SHA1Update(&ctx,key_header->value,strlen(key_header->value));
	SHA1Update(&ctx,ws_uuid,strlen(ws_uuid));
...
static int http_ws_cgi_execute(http_connection * c)
{
	WS_DBG("http_ws_cgi_execute c =%p",c);
	...
	else
	{
		...
		{
			WS_DBG("websocket frame size %d",c->body.len);
...
static void http_ws_connect_callback(void *arg)
{
...
void http_ws_server_init(http_callback connect,http_callback disconnect,http_callback received,http_callback data_sent)
{
	...
	WS_DBG("Websocket server init, conn=%p", &ws_config.server_conn);
}


void http_ws_server_start()
{
	WS_DBG("Websocket server start, conn=%p", &ws_config.server_conn);
