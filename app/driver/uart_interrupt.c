...
STATUS ICACHE_RAM_ATTR uart_tx_one_char_no_wait(uint8_t uart, uint8_t TxChar)
{
    uint8_t fifo_cnt = UART_TX_FIFO_COUNT(uart);
    if (fifo_cnt < 126)
      { UART_WRITE_CHAR(uart,TxChar); }
...
static void ICACHE_RAM_ATTR tx_timer_timeout(void *arg)
{
	if(arg==(void*)0)
      { uart_transmit(0); }
	else
...
void ICACHE_RAM_ATTR uart_clear_data_callback()
{
...
void ICACHE_RAM_ATTR uart0_write_char(char c)
{
   uart_write_char(0,c);
}
...
void uart_init(UartBautRate uart0_br, UartBautRate uart1_br)
{
   ...
   os_timer_setfn(&tx_timer[0], (os_timer_func_t *)tx_timer_timeout, (void *)0);

   os_memset(&tx_timer[1],0,sizeof(os_timer_t));
   os_timer_disarm(&tx_timer[1]);
   os_timer_setfn(&tx_timer[1], (os_timer_func_t *)tx_timer_timeout, (void *)1);
