...
class ELFFile:
    ...
    def _fetch_symbols(self):
        ...
        try:
            ...
            if (self.path is not ''):
                tool_nm = self.path + "/xtensa-lx106-elf-nm"
                if os.getenv('XTENSA_CORE')=='lx106':
                    tool_nm = self.path + "/xt-nm"
            else:
                tool_nm = "xtensa-lx106-elf-nm"
                if os.getenv('XTENSA_CORE')=='lx106':
                    tool_nm = "xt-nm"
    ...
    def get_entry_point(self):
        if (self.path is not ''):
            tool_readelf = self.path + "/xtensa-lx106-elf-readelf"
            if os.getenv('XTENSA_CORE')=='lx106':
                tool_objcopy = self.path + "/xt-readelf"
        else:
    ...
    def load_section(self, section):
        if (self.path is not ''):
            tool_objcopy = self.path + "/xtensa-lx106-elf-objcopy"
            if os.getenv('XTENSA_CORE')=='lx106':
                tool_objcopy = self.path + "/xt-objcopy"
        else:
...
def main():
    ...
    parser_elf2image.add_argument('--path', '-p', help = 'Set path of elf2bin',  type = str, default='')
    ...
    if args.operation == 'load_ram':
        ...
        e = ELFFile(args.input, args.path)
        image = ESPFirmwareImage()
