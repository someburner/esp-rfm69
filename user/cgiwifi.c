...
/*
 * ----------------------------------------------------------------------------
 * Modified and enhanced by Jeff Hufford in 2015
 * ----------------------------------------------------------------------------
 */
...
void ICACHE_FLASH_ATTR statusWifiUpdate(uint8_t state) {
  ...
  if ((wifi_get_opmode() & 0x3) == 0x01) {
    wifiStatePersist = 0x01;
    os_printf("Internet connection acquired!\n");
  } else if (wifiStatePersist == 0x01) {
    wifiStatePersist = 0x00;
    os_printf("Internet connection lost!\n");
  }
}
...
int ICACHE_FLASH_ATTR cgiWiFiSpecial(HttpdConnData *connData) {
	...
	if (os_strcmp(dhcp, "off") == 0) {
		...
		if (hl == 0) os_strcpy(hostname, "esp-rfm69");
